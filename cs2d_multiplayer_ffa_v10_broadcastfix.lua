-- ============================================================================
-- 2D CS-LIKE MULTIPLAYER FFA v10 • BROADCAST STATE FIX
--
...
--     4) общие broadcast-каналы лобби, матча и отказов.
--
...
local WORLD_W = 3600
local WORLD_H = 2400
local PROTOCOL_VERSION = "CSFFA10"
local ROOT_CHANNEL = "cs2d_ffa_multiplayer_v10"
...
local PERF = {
    ...
    -- Хост -> все клиенты: общие broadcast-каналы. Они работают по той же
    -- схеме, что и маяк хоста, и не зависят от персонального to_<pc_id>.
    lobby_channel = ROOT_CHANNEL .. "_lobby_broadcast",
    match_channel = ROOT_CHANNEL .. "_match_broadcast",
    reject_channel = ROOT_CHANNEL .. "_reject_broadcast",
    last_lobby_seq = -1,
    last_match_seq = -1,
    last_reject_seq = -1
}
...
-- ОКНО
-- ---------------------------------------------------------------------------
local win = create_window("2D CS • MULTIPLAYER FFA v10 • BROADCAST FIX", START_W, START_H)
...
local function send_lobby_to_clients()
    ...
    -- Один общий пакет вместо отдельного канала для каждого ПК.
    localhost_send(PERF.lobby_channel, join({
        "L",
        PROTOCOL_VERSION,
        host_session,
...
local function send_state_to_clients()
    ...
    -- Broadcast-снимок видят все клиенты одной сессии. Это одновременно
    -- дешевле и надёжнее, чем N персональных send-вызовов.
    localhost_send(PERF.match_channel, join({
        "S",
        PROTOCOL_VERSION,
        host_session,
        tostring(host_packet_seq),
...
local function send_reject(client_id)
    host_packet_seq = host_packet_seq + 1
    localhost_send(PERF.reject_channel, join({
        "R",
        PROTOCOL_VERSION,
        host_session,
        tostring(host_packet_seq),
...
local function become_client(beacon)
    ...
    PERF.last_lobby_seq = -1
    PERF.last_match_seq = -1
    PERF.last_reject_seq = -1
    PERF.joined_lobby = false
...
local function process_host_beacon()
    ...
    if role == "client" and beacon.session == host_session then
        ...
        -- Уже принятый игрок переходит в матч по самому маяку. Полный снимок S
        -- придёт по отдельному broadcast-каналу следом. Поэтому потеря одного
        -- стартового STATE больше не оставляет клиента навечно в лобби.
        if beacon.phase == "match" and PERF.joined_lobby then
            phase = "match"
            if client_players[self_id] == nil then
                client_players[self_id] = new_player(self_id, local_nick)
            end
        end
    end
end
...
local function host_process_join()
    ...
    if phase == "match" then
        if player ~= nil then
            -- Это уже участник текущего матча. Только НОВЫЙ JOIN-счётчик
            -- считается heartbeat; повторно читаемый старый пакет время не обновляет.
            if counter > player.last_join_counter then
                player.last_join_counter = counter
                player.nick = client_nick
                player.last_seen = time()
                player.connected = true
                if not player.alive and (player.respawn_at or 0) <= 0 then
                    choose_respawn(player)
                end
            end
            return
    ...
    if player == nil then
        ...
        if player == nil then
            ...
            localhost_send(PERF.reject_channel, join({
                ...
                tostring(host_packet_seq), tostring(client_id), "FULL", host_nick
            }, "|"))
            return
        end
    end
                ...
                if #fields >= 11 and
                   ...
                   fields[4] == id then
                    ...
                    if seq > player.last_input_seq then
                        local was_disconnected = not player.connected
                        player.last_input_seq = seq
                        player.last_seen = time()
                        player.connected = true
                        if was_disconnected and not player.alive and (player.respawn_at or 0) <= 0 then
                            choose_respawn(player)
                        end
...
local function client_process_host_packet()
    ...
    -- Лобби читается из общего broadcast-канала. Каждый клиент фильтрует
    -- protocol/session и применяет только новый seq.
    if phase == "lobby" or phase == "discovering" then
        local packet = localhost_receive(PERF.lobby_channel)
        if packet ~= nil and packet ~= "" then
            local fields = split(packet, "|")
            if #fields >= 7 and fields[1] == "L" and
               fields[2] == PROTOCOL_VERSION and fields[3] == host_session then
                local seq = tonumber(fields[4]) or -1
                if seq > PERF.last_lobby_seq then
                    PERF.last_lobby_seq = seq
                    last_host_packet_seq = math.max(last_host_packet_seq, seq)
                    last_host_packet_at = time()
                    phase = "lobby"
    ...
    -- Как только маяк сообщил MATCH, клиент начинает читать отдельный канал
    -- снимков матча. Он не конфликтует ни с лобби, ни с REJECT.
    if phase == "match" or PERF.last_beacon_phase == "match" then
        local packet = localhost_receive(PERF.match_channel)
        if packet ~= nil and packet ~= "" then
            local fields = split(packet, "|")
            if #fields >= 8 and fields[1] == "S" and
               fields[2] == PROTOCOL_VERSION and fields[3] == host_session then
                local seq = tonumber(fields[4]) or -1
                if seq > PERF.last_match_seq then
                    PERF.last_match_seq = seq
                    last_host_packet_seq = math.max(last_host_packet_seq, seq)
                    last_host_packet_at = time()
                    phase = "match"
                    PERF.joined_lobby = true
    ...
    -- Отклонения тоже broadcast, но содержат target PC ID.
    local reject_packet = localhost_receive(PERF.reject_channel)
    if reject_packet ~= nil and reject_packet ~= "" then
        local fields = split(reject_packet, "|")
        if #fields >= 7 and fields[1] == "R" and
           fields[2] == PROTOCOL_VERSION and fields[3] == host_session and
           fields[5] == self_id then
            local seq = tonumber(fields[4]) or -1
            if seq > PERF.last_reject_seq then
                PERF.last_reject_seq = seq
                local reason_code = fields[6]
                local remote_host_nick = fields[7]

                -- Уже принятый участник текущей сессии не становится late join.
                if not (reason_code == "RUNNING" and PERF.joined_lobby) then
...
print("CS2D MULTIPLAYER FFA v10 BROADCAST-FIX started")
