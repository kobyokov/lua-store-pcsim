-- ============================================================================
-- 2D CS-LIKE MULTIPLAYER FFA v5 OPTIMIZED
--
...
local WORLD_W = 3600
local WORLD_H = 2400
local PROTOCOL_VERSION = "CSFFA5"
local ROOT_CHANNEL = "cs2d_ffa_multiplayer_v5"
...
-- ОКНО
-- ---------------------------------------------------------------------------
local win = create_window("2D CS • MULTIPLAYER FFA v5 OPTIMIZED", START_W, START_H)
...
local function update_player_render_positions()
    if role == "client" then
        for id, player in pairs(client_players) do
            local factor = id == self_id and 0.48 or 0.32
            player.render_x = lerp(player.render_x or player.x, player.x, factor)
            player.render_y = lerp(player.render_y or player.y, player.y, factor)
        end
    else
...
print("CS2D MULTIPLAYER FFA v5 OPTIMIZED started")
